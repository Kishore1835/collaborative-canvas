...
socket.on("drawing-start", (data) => {
    console.log("REMOTE DRAWING START:", data);
    remoteLastPoints[data.userId] = {
        x: data.x,
        y: data.y
    };
});
...
socket.on("drawing", (data) => {
    ...
    const previousPoint = remoteLastPoints[data.userId];
    if (!previousPoint) {
        remoteLastPoints[data.userId] = {
            x: data.x,
            y: data.y
...
socket.on("canvas-state", (data) => {
    ...
    console.log("CANVAS REVISION:", data.revision);
    // Ignore an older canvas state
    if (data.revision < latestRevision) {
        console.log("IGNORING STALE CANVAS STATE:", data.revision, "Current:", latestRevision);
        return;
    }
    // Store the newest revision
    latestRevision = data.revision;
    Object.keys(remoteLastPoints).forEach((userId) => {
        delete remoteLastPoints[userId];
    });
...
function createRemoteCursor(userId, color) {
    ...
    const parentStyle = getComputedStyle(canvasParent);
    if (parentStyle.position ===
        "static") {
        canvasParent.style.position =
            "relative";
    }
    canvasParent.appendChild(cursor);
...
socket.on("cursor-move", (data) => {
    ...
    if (!cursor) {
        cursor =
            createRemoteCursor(data.userId, data.color ||
                "#3b82f6");
    }
    // ==================================================
    // Move cursor
