...
app.get("/main.js", (_req, res) => {
    res.sendFile(path.join(process.cwd(), "dist/client/main.js"));
});
app.get("/canvas.js", (_req, res) => {
    res.sendFile(path.join(process.cwd(), "dist/client/canvas.js"));
});
app.get("/websocket.js", (_req, res) => {
    res.sendFile(path.join(process.cwd(), "dist/client/websocket.js"));
});
// ==================================================
// HEALTH
// ==================================================
...
function isValidDrawingData(data) {
    ...
    return (isValidNumber(drawingData.x) &&
...
function sendUsersUpdate() {
    const userList = Array.from(users.entries()).map(([id, color]) => ({
        id,
        color
    }));
...
io.on("connection", (socket) => {
    ...
    users.set(socket.id, assignedColor);
    console.log(`Assigned color ${assignedColor} to ${socket.id}`);
    sendUsersUpdate();
    // ==================================================
    ...
    socket.on("request-canvas-state", () => {
        console.log(`Canvas state requested by ${socket.id}`);
        socket.emit("canvas-state", {
            strokes,
            revision: canvasRevision
        });
    ...
    socket.on("drawing-start", (data) => {
        ...
        if (!isValidDrawingData(data)) {
            console.warn(`Invalid drawing-start received from ${socket.id}`);
            return;
        }
        const stroke = {
        ...
        if (!isValidStroke(stroke)) {
            console.warn(`Generated invalid stroke from ${socket.id}`);
            return;
        }
    ...
    socket.on("drawing", (data) => {
        // Validate incoming drawing data
        if (!isValidDrawingData(data)) {
            console.warn(`Invalid drawing data received from ${socket.id}`);
            return;
        }
        ...
        if (!stroke) {
            console.warn(`Drawing point received without active stroke from ${socket.id}`);
            return;
        }
        stroke.points.push({
            x: data.x,
    ...
    socket.on("drawing-end", () => {
        ...
        if (!isValidStroke(stroke)) {
            console.warn(`Invalid completed stroke from ${socket.id}`);
    ...
    socket.on("cursor-move", (data) => {
        ...
        if (typeof data !== "object" ||
            data === null) {
            console.warn(`Invalid cursor data received from ${socket.id}`);
            return;
        }
        const cursorData = data;
        if (!isValidNumber(cursorData.x) ||
            !isValidNumber(cursorData.y)) {
            console.warn(`Invalid cursor coordinates received from ${socket.id}`);
            return;
        }
        const userColor = users.get(socket.id) ||
            "#3b82f6";
        console.log("Cursor received:", {
            userId: socket.id,
            x: cursorData.x,
            y: cursorData.y,
            color: userColor
    ...
    socket.on("clear-canvas", () => {
        console.log(`Clear requested by ${socket.id}`);
        strokes = [];
        redoStack = [];
        canvasRevision++;
        broadcastCanvas();
